...
class PortfolioConfig:
    ...
    max_gross: float = 1.0  # gross exposure cap; 1.0 = unlevered
    max_weight_per_asset: Optional[float] = None  # e.g., 0.2 means <=20% per asset; None disables
    cash_buffer: float = 0.0  # e.g., 0.02 means keep 2% in cash
    ...
    # Integer shares (your requirement)
    allow_fractional_shares: bool = False  # keep False
...
class PortfolioEngine:
    ...
    def run(
        ...
        for i in range(len(idx) - 1):
            ...
            # 6) Apply constraints (gross cap, per-asset cap, cash buffer)
            target_weights = self._apply_constraints(target_weights)

            # 7) Size to integer shares using open(t+1) (consistent with fill price)
            ...
            # 8) Create orders (delta shares) and simulate fills at open(t+1)
            for s in symbols:
                ...
                fill_price = float(open_t1[s])  # next_open model
